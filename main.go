...
func fixupTargetTags(tags []string) []string {
	tags = slices.Clone(tags)
	if !slices.Contains(tags, "gc") && !slices.Contains(tags, "gccgo") {
		tags = append(tags, runtime.Compiler)
	}
	if !slices.Contains(tags, "unix") && slices.ContainsFunc(tags, func(tag string) bool { return unixOS[tag] }) {
		tags = append(tags, "unix")
	}
	if slices.Contains(tags, "android") && !slices.Contains(tags, "linux") {
		tags = append(tags, "linux")
	}
	if slices.Contains(tags, "illumos") && !slices.Contains(tags, "solaris") {
...
func main() {
	flg := flag.NewFlagSet("ryegen", flag.ExitOnError)
	var optClean = flg.Bool("clean", false, "delete Go files generated by Ryegen (pwd by default, or you can specify the directories as args)")
	var optQuiet = flg.Bool("q", false, "quiet: hide warnings")
	var optVerbose = flg.Bool("v", false, "verbose output")
	var optGOOS = flg.String("goos", "", "target operating system (default: current GOOS)")
	var optGOARCH = flg.String("goarch", "", "target CPU architecture (default: current GOARCH)")
	var optTags []string
	flg.Var(TagsValue{V: &optTags}, "tags", "additional target build tags (separated by ,)")
	flg.Usage = func() {
		fmt.Fprintln(flg.Output(), "Usage:")
		fmt.Fprintln(flg.Output(), "\t"+flg.Name(), "[flags] [directory with ryegen.toml]")
		fmt.Fprintln(flg.Output(), "Flags:")
		flg.PrintDefaults()
	}
	_ = flg.Parse(os.Args[1:])

	if *optGOOS == "" {
		*optGOOS = cmp.Or(os.Getenv("GOOS"), runtime.GOOS)
	}
	if *optGOARCH == "" {
		*optGOARCH = cmp.Or(os.Getenv("GOARCH"), runtime.GOARCH)
	}
	...
	args := flg.Args()
	if len(args) > 1 {
		fmt.Println("expected at most 1 positional argument")
		flg.Usage()
		os.Exit(2)
	}
	...
	ryegenTOML := "ryegen.toml"
	if len(args) == 1 {
		ryegenTOML = filepath.Join(args[0], ryegenTOML)
	}
	cfg, err := config.Load(ryegenTOML)
	if err != nil {
		if errors.Is(err, os.ErrNotExist) {
			...
			logger.Log(FATAL, "%s not found (working directory: %s)", ryegenTOML, wd)
	...
	{
		explicitTags := append([]string{*optGOOS, *optGOARCH}, optTags...)
		targetName = strings.Join(explicitTags, "_")

		tags := fixupTargetTags(explicitTags)
